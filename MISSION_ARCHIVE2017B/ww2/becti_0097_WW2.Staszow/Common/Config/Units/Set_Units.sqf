	...
	if (isClass (configFile >> "CfgVehicles" >> _classname)) then {
		...
		if (isNil "_get") then {
			...
			if (_picture == "") then { 
				...
				if (_picture == "pictureThing") then {_picture = ""}; //--- If vehicle only has a placeholder picture (*cough*Huron containers*cough*) then remove it
			};
			_label = if ((_n select _i) == "") then { getText(configFile >> "CfgVehicles" >> _classname >> "displayName") } else { _n select _i };
			_turrets = if !(_classname isKindOf "Man") then { (_classname) call CTI_CO_FNC_GetVehicleTurrets } else { "" };
			...
			_stored = [
				_label,
				_picture,
				if (CTI_DEBUG) then {1} else {_o select _i},
				if (CTI_DEBUG) then {1} else {_t select _i},
				_u select _i,
				_f select _i,
				_turrets,
				_scripts
