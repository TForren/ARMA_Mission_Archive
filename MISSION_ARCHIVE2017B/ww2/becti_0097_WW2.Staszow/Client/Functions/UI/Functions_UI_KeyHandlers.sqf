CTI_UI_KeyHandler_BuildMenu_KeyDown = {
	private ["_key"];
	_key = _this select 1;
...
CTI_UI_KeyHandler_BuildMenu_KeyUp = {
	private ["_key"];
	_key = _this select 1;
...
CTI_UI_KeyHandler_BuildMenu_MouseButtonDown = {
	private ["_control", "_button"];
	_button = _this select 1;
	_control = _this select 4;
	
	switch (_button) do {
		case (0): {if (!CTI_VAR_StructureProhibit && CTI_P_PreBuilding_SafePlace) then {CTI_VAR_StructurePlaced = true}};
		case (1): {CTI_VAR_StructureCanceled = true};
	};
};
