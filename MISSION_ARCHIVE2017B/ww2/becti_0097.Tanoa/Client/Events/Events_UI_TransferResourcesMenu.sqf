	...
	case "onGivePlayerPressed": {
		...
		if (_amount > 0 && _amount <= call CTI_CL_FNC_GetPlayerFunds) then {
			...
			if (_recievingGroup != group player) then {
				...
				[["CLIENT", leader _recievingGroup], "Client_OnMessageReceived", ["funds-transfer", [_amount ,_sendingGroup]]] call CTI_CO_FNC_NetSend;
				hint parseText format ["<t size='1.3' color='#2394ef'>Information</t><br /><br />Transfered <t color='%1'>$%2</t> to group <t color='#55bcfc'>%3</t>.", CTI_P_Coloration_Money, _amount, _recievingGroup getVariable ["cti_alias",CTI_PLAYER_DEFAULT_ALIAS]];
