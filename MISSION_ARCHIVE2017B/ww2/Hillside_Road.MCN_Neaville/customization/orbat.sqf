...
// Loop through the group, print out group ID, leader name and medics if present
{
    ...
    _leaderRole01 = roleDescription _groupleader;
 
    _leaderRole02 = _leaderRole01 splitString "@";
 
    _leaderRole03 = _leaderRole02 select 0;
 
    _orbatText = _orbatText + format ["<font color='%3'>%1 - %2</font>", _leaderRole03, name leader _x,_color] + "<br />";
 
   
    
    {
		_unitloadout = (_x getVariable ["FW_Loadout", ""]);
        if (["MED",_unitloadout] call BIS_fnc_inString && {_x != leader group _x}) then {
    ...
    {
		 _unitloadout = (_x getVariable ["FW_Loadout", ""]);
        if ( ["FTL",_unitloadout] call BIS_fnc_inString  && {_x != leader group _x}) then {
