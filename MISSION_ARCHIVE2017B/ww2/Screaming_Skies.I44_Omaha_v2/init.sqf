...
if (isServer) then {
	...
	[] spawn { //Spawns code running in parallel
		...
		while {!FW_MissionEnded} do { //Loops while the mission is not ended
			...
			indCasualty = "101st Airborne" call FNC_CasualtyPercentage; //Gets the casualty percentage of team "MSV"
			publicVariable "indCasualty";

			westCasualty = "Germany" call FNC_CasualtyPercentage; //Gets the casualty percentage of team "USA" based off the team names from setting.sqf
			...
			if ((time / 60) >= FW_TimeLimit && FW_TimeLimit != 0) exitWith { //It is recommended that you do not remove the time limit end condition 
				...
				format ["The mission is complete.<br/><br/>%1x AA destroyed.<br/>%2x supply trucks destroyed.<br/>%3x German HVTs killed.<br/>",({!alive _x} count AAUnits),destroyedTrucks,({!alive _x} count HVTs)] call FNC_EndMission;
