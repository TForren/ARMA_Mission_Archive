...
if (_req_target != _req_buyer && isPlayer leader _req_target) then { //--- Notify the remote target
	[["CLIENT", leader _req_target], "Client_OnPurchaseOrderCancelled", [_req_seed, _req_classname, _factory, _req_unit_count]] call CTI_CO_FNC_NetSend;
};