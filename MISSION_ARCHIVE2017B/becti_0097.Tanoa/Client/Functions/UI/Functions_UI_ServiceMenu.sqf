...
CTI_UI_Service_ProcessRepair = {
	...
	while {true} do {
		...
		if (time - _start_at > _service_time || CTI_DEBUG) exitWith {_repair = true};
...
CTI_UI_Service_ProcessRearm = {
	...
	while {true} do {
		...
		if (time - _start_at > _service_time|| CTI_DEBUG) exitWith {_rearm = true};
...
CTI_UI_Service_ProcessRefuel = {
	...
	while {true} do {
		...
		if (time - _start_at > _service_time || CTI_DEBUG) exitWith {_refuel = true};
