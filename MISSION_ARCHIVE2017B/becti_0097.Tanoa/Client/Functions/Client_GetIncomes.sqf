...
if (_total_values select 0 == 0) exitWith {[0,0]}; //--- Don't bother if we have no towns!
...
_pool_award = (_total_values select 0) * _percent_award;

_pool_income_total = (_total_values select 0) - _pool_award; //--- The resources pool overall
