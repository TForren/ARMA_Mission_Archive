...
HZ_fnc_getClosest = {
	...
	_closestDude = "";
	{
		if ( side _x == east && ((exitDoor distance _x) < _shortest)) then {
...
HZ_fnc_beginHunt = {
	_this spawn {
		while {true} do {
			_group = _this;
			//_avgLoc = [[east,false]] call HZ_fnc_avgSideLoc;
	...
	while {!alerted} do {
		{
			if (behaviour _x == "COMBAT") then {
				startTime = time;
				waitUntil{sleep 1; (time - startTime > (60 * 10)) || (triggeractivated ophere)  };
				alerted = true;
				publicVariable "alerted";
				[] call HZ_fnc_callReinforce;
				//[] call HZ_fnc_heloSearch;
			};
		} foreach ([leader guardGroup1] + [leader guardGroup2] + [leader guardGroup3] + [leader guardGroup4] + [leader guardGroup5] + [leader guardGroup6]);
		sleep 5;
	};
};
