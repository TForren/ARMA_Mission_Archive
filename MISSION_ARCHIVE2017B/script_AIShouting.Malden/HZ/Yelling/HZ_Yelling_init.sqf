...
HZ_fnc_AIShootingYell = compile preprocessfilelinenumbers "HZ\Yelling\HZ_fnc_AIShootingYell.sqf";
...
#include "voices\greekVoices.sqf";

{
	if ((side _x == _faction) && !(_x in excludeUnits)) then {
		...
		_yellEH = _x addEventHandler ["fired", {[(_this select 0)] call HZ_fnc_AIShootingYell}];
