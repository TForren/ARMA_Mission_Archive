if (isServer) then { //This scope is only for the server
	...
	FW_TimeLimit = 10; //Time limit in minutes, to disable the time limit set it to 0
...
if (!isDedicated) then { //This scope is only for the player
	...
	FW_RespawnTickets = 9999;//If respawn is enabled you must create empty game logics, for respawn points, following the name format fw_side_respawn. Example: fw_west_respawn
};
