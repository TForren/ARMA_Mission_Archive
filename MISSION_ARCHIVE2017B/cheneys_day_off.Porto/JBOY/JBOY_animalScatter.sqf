...
while {alive _critter} do
{
    ...
    if (isnull (attachedTo _critter)) then  // if boost not already running on this critter
    {
        ...
        _men = [];
        {
            if ((speed _x) > 1) then {_vehicles pushback _x;};
        } foreach (nearestObjects [ getpos _critter,["LandVehicle"],15]);
        if (count _vehicles == 0) then
        {
            {  // scatter critters when men close and moving.  IF prone, don't scatter until man very close.
                if ((speed _x) > 3 or ((eyePos _x select 2 )- (getPosAsl _x select 2))> .5 or (_x distance _critter) < 1.2) then 
                {
                    _men pushback _x;
                };
                
            } foreach (nearestObjects [ getpos _critter,["CAManBase"],7]);
            //_men = nearestObjects [ getpos _critter,["CAManBase"],7]; 
        };
        if (count _men == 0 and count _vehicles == 0) then
        {
            _dogs = nearestObjects [ getpos _critter,["Dog_Base_F"],7]; 
        }; 
        if (count _dogs > 0 or count _men > 0 or count _vehicles > 0) then
        {
    //hint format ["Scatter, type of %1",typeOf _critter];
            _threat = (_men + _dogs + _vehicles) select 0;
            switch (typeOf _critter) do 
