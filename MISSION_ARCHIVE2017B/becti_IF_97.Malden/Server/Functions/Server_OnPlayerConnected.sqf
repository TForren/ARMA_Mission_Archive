...
if ((missionNamespace getVariable "CTI_AI_TEAMS_JIP_PRESERVE") == 0) then {
	if (CTI_Log_Level >= CTI_Log_Information) then {["INFORMATION", "FILE: Server\Functions\Server_OnPlayerConnected.sqf", format["Team [%1] units are now being removed for player [%2] [%3].", _team, _name, _uid]] call CTI_CO_FNC_Log};
