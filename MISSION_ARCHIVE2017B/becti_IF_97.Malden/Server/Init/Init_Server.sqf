...

{
	...
	//--- Startup vehicles
	{
		...
		diag_log format ["DOING STUFF WITH %1",_x select 0];
		_var = missionNameSpace getVariable _model;
		diag_log format ["TRYING STUFF WITH %1",missionNameSpace getVariable _model];
		...
		//diag_log format ["Got the _var: %1",_var];
		//diag_log format ["Got the _script: %1",_script];
