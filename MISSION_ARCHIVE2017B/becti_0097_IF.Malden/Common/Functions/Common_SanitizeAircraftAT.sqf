...
//--- Find AT Lock weapons.
{
	...
	
	{
		...
		if (_ammo != "") then {
			...
			//--- We check if the ammo is ir-lock based and that in inherits from the missile class.			
			if (getNumber(configFile >> "CfgAmmo" >> _ammo >> "irLock") == 1 && getNumber(configFile >> "CfgAmmo" >> _ammo >> "airLock") == 0 && _ammo isKindOf "MissileBase") then {
				_remove = true; 
...
//--- Remove all AT related magazines and weapons
{
