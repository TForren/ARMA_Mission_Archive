...
waitUntil {!isNil "modCheckDone"};
waitUntil {modCheckDone};
...
_obj addAction [format ["<t color='#2080ff'>%1</t>", localize "STR_ResumeSpectating"], "spectate.sqf", [""], 15, false, true, "",  "roundInProgress && (!isPlaying) && (!(player getVariable ['isPlaying', false]))"];
