...
if(vas_disableLoadSave) then {
	_loadout = missionNamespace getVariable format["PO3_VAS_gearSlot_%1_%2_%3",side player, player getVariable ["PO3_VAR_roleAttribute","Rifleman"],_slot];
}else{
	_loadout = profileNamespace getVariable format["PO3_VAS_gearSlot_%1_%2_%3",side player, player getVariable ["PO3_VAR_roleAttribute","Rifleman"],_slot];
};
