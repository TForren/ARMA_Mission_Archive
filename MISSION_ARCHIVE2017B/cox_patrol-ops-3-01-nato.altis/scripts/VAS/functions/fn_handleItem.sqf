...
if( _item in PO3_CfgLogistics_UnitLoadoutBlacklist ) exitWith {
	systemChat format["%1 %2",_details select 1,localize "STR_VAS_restricted"];
};
...
{[_x,true,true,false,true] spawn VAS_fnc_handleItem; } foreach _items;
	...
	switch((_details select 6)) do {
		case "CfgVehicles" : { removeBackpack player; };
...
[player,true] call PO3_fnc_getLoadout;
