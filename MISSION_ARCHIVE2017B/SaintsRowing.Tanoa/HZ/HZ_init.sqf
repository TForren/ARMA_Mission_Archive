...
HZ_fnc_getClosest = {
	...
	_closestDude = "";
	{
		if ( side _x == west && ((exitDoor distance _x) < _shortest)) then {
			_shortest = (exitDoor distance _x);
			_closestDude = _x;
		};
	} foreach allunits;
	_closestDude;
	...
	while {!alerted} do {
		{
			if (behaviour _x == "COMBAT") then {
				startTime = time;
				{ _x setBehaviour "COMBAT" } foreach ([garrisongroup] + [garrisongroup2] + [garrisongroup3] + [garrisongroup4] + [garrisongroup5]);
				waitUntil{sleep 1; (time - startTime > (60 * 10)) || ({alive _x} count [helo1,helo2] < 2)  };
				alerted = true;
				publicVariable "alerted";
				[] call HZ_fnc_callReinforce;
				[] call HZ_fnc_heloSearch;
			};
		} foreach ([leader garrisongroup] + [leader garrisongroup2] + [leader garrisongroup3] + [leader garrisongroup4] + [leader garrisongroup5]);
		sleep 5;
	};
};
