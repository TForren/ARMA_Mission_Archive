...
switch _mode do {
	...
	case "spawnVehicle": {
		...
		if ( count nearestObjects [ _pos, [ "Car", "Tank", "Air", "Ship" ], 10 ] > 0 ) exitWith {
			...
			[ "NMD Vehicle Spawner", "Unable to spawn vehicle near other vehicles" ] call NMD_fnc_titleHint;
		...
		//--- Move player in vehicle
		if ( ctrlChecked _ctrlCheckMoveIn ) then {
		...
		if ( NMD_ModuleVehicles_garageLimit > 0 && { count NMD_ModuleVehicles_spawned > NMD_ModuleVehicles_garageLimit } ) then {
			...
			[ 
				...
				"NMD Vehicle Spawner",				
				format[ "Removed %1", getText ( configFile >> "CfgVehicles" >> typeOf ( NMD_ModuleVehicles_spawned select 0 ) >> "displayName" ) ] 
