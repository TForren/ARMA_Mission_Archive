...
_last_commander = (CTI_P_Sidejoined) call CTI_CO_FNC_GetSideCommander;
...
while { true } do {
	...
	if (time - _last_infocheck > 1 || uiNamespace getVariable "cti_dialog_ui_transferresourcesmenu_forceupdate") then {		
		...
		_groups = uiNamespace getVariable "cti_dialog_ui_transferresourcesmenu_groups";

		if (_last_commander != _commander && !isNull _commander) exitWith { //--- Commander change, reload the UI.
			{lnbdeleterow [140001,0]} forEach _groups;
			['onLoad'] call compile preprocessFileLineNumbers 'Client\Events\Events_UI_TransferResourcesMenu.sqf';
			_last_commander = _commander;
		};
