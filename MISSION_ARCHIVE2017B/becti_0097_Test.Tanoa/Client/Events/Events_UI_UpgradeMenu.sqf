		...
		if (call CTI_CL_FNC_IsPlayerCommander) then {
			if (_selected > -1) then {
				if ((CTI_P_SideLogic getVariable "cti_upgrade") < 0) then {
					...
					_upgrade_prices = ((missionNamespace getVariable Format["CTI_%1_UPGRADES_COSTS", CTI_P_SideJoined]) select _selected) select _upgrade_current;
					if (CTI_DEBUG) then {_upgrade_prices = [1,1]};
					...
					if (_upgrade_current < _upgrade_level) then {
						if (_funds >= (_upgrade_prices select 1) ) then {	
							if (!_supplyActive || (_supply >= (_upgrade_prices select 0))) then {
								...
								if !(_link_needed) then {
									-(_upgrade_prices select 1) call CTI_CL_FNC_ChangePlayerFunds;
									if (_supplyActive) then {[CTI_P_SideJoined, -(_upgrade_prices select 0)] call CTI_CO_FNC_ChangeSideSupply;};
