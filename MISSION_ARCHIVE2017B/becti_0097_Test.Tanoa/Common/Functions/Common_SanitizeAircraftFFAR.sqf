...
//--- Find FFAR weapons.
{
	...
	
	{
		...
		if (_ammo != "") then {
			...
			//--- We check if the ammo inherits from the missile or rocket class.
			if (getNumber(configFile >> "CfgAmmo" >> _ammo >> "irLock") == 0 && (_ammo isKindOf "MissileBase" || _ammo isKindOf "RocketBase")) then { //--- Pawnee/Hellcat DARs were changed from rocketbase to missilebase at some point but MI48 DARs are still rocketbase. Figures.
				_remove = true; 
...
//--- Remove all FFAR related magazines and weapons
{
