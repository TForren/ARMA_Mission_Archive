...
Harcout, Lijnhaven, Kotomo, Northern sawhill.
<br/><br/>
<font color='#FFA500'>MLCOA:</font><br/>
Harcourt CSAT are likely stationed in defensive post but otherwise no patrols or expectation of contact.<br/>
Reinforcements from the North and South will likely be on scene to support Harcout 10-15 into engagement.
<br/>
...
<br/>10x M72s in Boxer Truck
ENDTAB;

NEWTAB("II. Mission:")
RAID the Harcourt CSAT post and DESTROY CSAT AA assets. ESCAPE CSAT reinforcements via underground tunnel system.
ENDTAB;

NEWTAB("III. Execution:")

<font color='#FFA500'>a. Concept of the Operation</font><br/>
Insurgent Raiders will clear the CSAT post to rig the AA assets with explosives or hit the AA assets from a distance with AT weapons. They will then escape CSAT reinforcements via tunnel system.
<br/>
<br/>
...
// change this around to fit your mission. * indicates your unit
		...
		<br/>
		<br/>	'Musk is flying home' - AA is rigged to explode.
		<br/>
...
<br/>
<br/>Victory is achieved when the AA is destroyed and raiders have escaped.
<br/>
<br/>Minor victory is achieved when the AA is destroyed but raiders are eliminated.
<br/>
<br/>Failure occurs if the AA is not destroyed and the raiders are eliminated.
<br/>
ENDTAB;
...
if (_adminState != 0 || _uid == "76561198013584551" || _uid == "76561197988578942") then {
	...
	<br/><execute expression='victoryMessage call FNC_EndMissionRequest'>Escaped and AA destroyed</execute>  
	<br/>
	<br/><execute expression='victoryMessage2 call FNC_EndMissionRequest'>No escape but AA destroyed</execute>  
	<br/>
	<br/><execute expression='stalemate call FNC_EndMissionRequest'>Escaped but active AA</execute>  
	<br/>
	<br/><execute expression='failureMessage call FNC_EndMissionRequest'>No escape active AA</execute>
	ENDTAB;
};
