...
*/
	...
	if (typeName _cond == "SCALAR") then {
		...
		[{!alive (_this select 0) || behaviour (_this select 0) == "COMBAT" || damage (_this select 0) > (_this select 1) || (_this select 0) call BIS_fnc_enemyDetected || _cond}, {(_this select 0) setVariable ["ambientAnimMonitor_exit",true,false]}, [_unit,_hurt]] call CBA_fnc_waitUntilAndExecute;
	};
