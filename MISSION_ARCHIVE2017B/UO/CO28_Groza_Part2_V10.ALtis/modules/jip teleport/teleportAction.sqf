...
if (_target getvariable ["ACE_isUnconscious",false] || _target getVariable ["frameworkDead", false] || !(alive _target)) then {
	_rank = -1;

	{
		if (rankId _x > _rank && (_x getvariable ["ACE_isUnconscious",false] == false)) then {
			_rank = rankId _x;
			_target = _x;
		};
