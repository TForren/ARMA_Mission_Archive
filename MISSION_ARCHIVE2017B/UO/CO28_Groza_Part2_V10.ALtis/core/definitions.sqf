...
_tempAO = []; \
{if (CONDITION) then {_tempAO set [count _tempAO, _x];};} forEach ARRAY; \
VARNAME = _tempAO;
...
_tempAO = []; \
{_tempAO set [count _tempAO, (_x getVariable VARIABLE)];} forEach ARRAY; \
VARNAME = _tempAO;
...
QUERY(_countQuery, _x getvariable ["ACE_isUnconscious",false] == false, _countQuery); \
...
QUERY(_areaQuery, _x getvariable ["ACE_isUnconscious",false] == false, _areaQuery); \
...
if (isMultiplayer) then { \
	TEAMSTEST set [count TEAMSTEST, [NAME, 0, 0, [], []]]; \
...
{ \
	if ((_x select 0) == TEAM) exitWith { \
		_tempAOStart = (_x select 1); \
		_tempAOCurrent = (_x select 2);\
		_tempAO = (_tempAOStart - _tempAOCurrent)/(_tempAOStart * 0.01); \
	}; \
} forEach TEAMSTEST; \
VARNAME = _tempAO;
...
["endScreen", [SCENARIO, timeLimit, TEAMSTEST]] call CBA_fnc_globalEvent;
