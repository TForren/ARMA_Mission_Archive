...
[{
    ...
    [{
        ...
        if ((floor ((({position _x inArea "mission1_1_mrk" && {side _x == east} && {alive _x}} count allUnits) / _mainAOUnitCount) * 100) <= ceil ((derp_PARAM_AOFinishEnemyPercentage / 100) * _mainAOUnitCount) && {!derp_sideMissionInProgress}) || (!isNil "ADMIN_SKIP" && {ADMIN_SKIP})) then {

            deleteMarker "mission1_mrk";
            deleteMarker "mission1_1_mrk";
            [_missionID, 'Succeeded', true] call BIS_fnc_taskSetState;
            ADMIN_SKIP = false;
            publicVariable "ADMIN_SKIP";
            missionWin = nil;
