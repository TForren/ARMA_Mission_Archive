...
//Adding eventhandlers
	"KRON_UPS_EAST_SURRENDED" addPublicVariableEventHandler { if (_this select 1) then { nul=[east] execvm "modules\UPSMON\MON_surrended.sqf";};};
	"KRON_UPS_WEST_SURRENDED" addPublicVariableEventHandler { if (_this select 1) then { nul=[west] execvm "modules\UPSMON\MON_surrended.sqf";};};
	"KRON_UPS_GUER_SURRENDED" addPublicVariableEventHandler { if (_this select 1) then { nul=[resistance] execvm "modules\UPSMON\MON_surrended.sqf";};};
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	if (isNil("KRON_UPS_INIT") || KRON_UPS_INIT == 0) then {
		...
		call compile preprocessFileLineNumbers "modules\UPSMON\common\MON_functions.sqf";
		
		//init !R functions
		call compile preprocessFileLineNumbers "modules\UPSMON\!R\R_functions.sqf";
		
		if (isNil "RE") then {[] execVM "\ca\Modules\MP\data\scripts\MPframework.sqf"}; 
		
		//scripts initialization
		UPSMON = compile preprocessFile "modules\UPSMON\UPSMON.sqf";	
		UPSMON_surrended = compile preprocessFile "modules\UPSMON\MON_surrended.sqf";	
