...
if (_eastCasualty >= 80) exitWith {
	if (_westCasualty <= 32) exitWith {
		"Mission End - Victory: The insurgents were destroyed and remnants retreated with minimal own casualties." call FNC_EndMission;
	};
...
if (obj1 == 1) then {
	if (_westCasualty <= 32) exitWith {
		"Mission End - Victory: The cache was secured by 3rd Platoon with minimal casualties." call FNC_EndMission;
	};
	if (_westCasualty >=33 and _westCasualty <=49) exitWith {
		"Mission End - Minor Defeat: The cache was secured, but 3rd Platoon is now combat ineffective." call FNC_EndMission;
	};
...
if ((time / 60) >= FW_TimeLimit && FW_TimeLimit != 0) exitWith { //It is recommended that you do not remove the time limit end condition 
	...
	if (_westCasualty >=33 and _westCasualty <=49) exitWith {
		...
		if (_westCasualty >= 50) exitWith {
		"Mission End - You Fucked Up: Time limit reached. 3rd platoon was destroyed. The platoon leader faces court martial." call FNC_EndMission;
	};
};
