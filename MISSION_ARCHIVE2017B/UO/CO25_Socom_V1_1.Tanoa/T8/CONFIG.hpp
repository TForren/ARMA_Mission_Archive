...
T8U_var_DirectCallRange			= 2000;			// group leader searches for help within XXX m
...
T8U_var_FiredEventTimeout		= 20;			// if fired near event triggered, pause it for XX sec ... spam reduce
T8U_var_CallForHelpTimeout		= 150;			// a group can only call one other group each x sec for help
T8U_var_SupportTimeout			= 150;			// every X sec a group (with a SL or Officer) can call in for a Support (e.g. para drop)
