...
if (isServer) then { //This scope is only for the server
	...
	setViewDistance 3000; //View distance for the server (the ai's)

	FW_TimeLimit = 90; //Time limit in minutes, to disable the time limit set it to 0
	FW_TimeLimitMessage = "TIME LIMIT REACHED!"; //The message displayed when the time runs out

	[west, "CTRG", "player"] call FNC_AddTeam; //Adds a player team called USMC on side west
	//[east, "Militia", "ai"] call FNC_AddTeam; //Adds a ai team called VDV on side east
	[resistance, "Militia", "ai"] call FNC_AddTeam; //Adds a player team called Local Militia on side resistance (aka independent)
