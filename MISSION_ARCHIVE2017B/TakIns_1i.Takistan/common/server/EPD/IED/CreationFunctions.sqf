...
[_sectionDictionary,_sectionName,_locationAndSize select 0,_parameters]call CREATE_SPECIFIC_IED;}else{
	if(_locationAndSize select 1 > 1)then{
...
if(count _parameters==3)then{
	...
	_sizeAndType=[_chances select 1,_chances select 2,_chances select 3]call GET_SIZE_AND_TYPE;}else{
	_chance=_parameters select 1;};
};
if(random 100 < _chance)then{
[_origin,_sizeAndType select 0,_sizeAndType select 1,_side,_dictionary,_sectionName]call CREATE_IED;}else{
...
switch(count _parameters)do{
	...
	if(typeName( _parameters select 2)=="ARRAY")then{
			...
			_junkToPlace=_junkToPlace + 1;}else{
			_iedsToPlace=_iedsToPlace + 1;};
			};
_chances=[_parameters select 2 select 1,_parameters select 2 select 2,_parameters select 2 select 3];}else{
_iedsToPlace=_parameters select 1;
...
_scriptHandle=[_ied,_sectionName,_iedName,_iedSize,_trigger]spawn PROJECTILE_DETECTION;}else{
...
_trigger setTriggerStatements['this && {["' + _sectionName + '","' + _iedName +'", thisList]call TRIGGER_CHECK}','["' + _sectionName + '","' + _iedName +'"]call EXPLOSIVESEQUENCE_' + _iedSize + ';',""];
...
_scriptHandle="";
if(allowExplosiveToTriggerIEDs)then{
_scriptHandle=[_ied,_sectionName,_iedName,"SECONDARY",_trigger]spawn PROJECTILE_DETECTION;}else{
_scriptHandle=0 spawn{};};
[_sectionDictionary,_iedName,[_ied,_trigger,_side,"SECONDARY",_markerName,_scriptHandle]]call ADD_IED_TO_SECTION;
_trigger setTriggerArea[11,11,0,true];
_trigger setTriggerActivation["any","PRESENT",false];
_trigger setTriggerStatements['this && {["' + _sectionName + '","' + _iedName +'", thisList]call TRIGGER_CHECK}','["' + _sectionName + '","' + _iedName +'"]call EXPLOSIVESEQUENCE_SECONDARY;',""];
