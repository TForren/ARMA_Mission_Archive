...
if ((side player) == blufor ) then //blufor
{
	_draw3DCallsigns = addMissionEventHandler ["Draw3D",{ //staging area name tags 
			...
			if ( (player distance (getmarkerpos "HQ")) < 100) then {
				...
				drawIcon3D ["", [0,1,0,1], [(getpos charlie) select 0,(getpos charlie) select 1,((getpos charlie) select 2) + 2.5], 0, 0, 0, "1'1 Charlie", 1, 0.04, "TahomaB"]; 
				
				drawIcon3D ["", [0.9,0.9,0.9,1], [(getpos cmd) select 0,(getpos cmd) select 1,((getpos cmd) select 2) + 2.5], 0, 0, 0, "M1A1 TUSK CMD", 1, 0.04, "TahomaB"]; 
...
if ((side player) == opfor ) then
{
	...
	_mkrBsvc4 = createMarkerLocal ["mkrBsvc4", getPos dealer];
	_mkrBsvc4 setMarkerTypeLocal "mil_dot";
	_mkrBsvc4 setMarkerTextLocal "Motor Pool";
