...

	{
...

		{
			if (alive _x) then {
				...
				lbSetColor [CSSA3_unitsListBoxIDC, _index, _colour];
				if (_x in CSSA3_favourites) then {lbSetPicture [CSSA3_unitsListBoxIDC, _index, "core\CSSA3\images\star.paa"];};
			};
		} count _unitsArray;
...
if (CSSA3_selectedTab == "Favourites") exitWith {
	...
	lbClear CSSA3_unitsListBoxIDC;
	{
		if (alive _x) then {
			...
			lbSetData [CSSA3_unitsListBoxIDC, _index, str _x];
			lbSetColor [CSSA3_unitsListBoxIDC, _index, _unitColour];
			lbSetPicture [CSSA3_unitsListBoxIDC, _index, "core\CSSA3\images\star.paa"];

//			if (CSSA3_spectatedUnit == _x) then {[_index] spawn CSSA3_fnc_setListboxIndex};
		};
	} count CSSA3_favourites;
};
...
if (CSSA3_selectedTab == "Blufor") exitWith {[west] call _updateSide};
if (CSSA3_selectedTab == "Opfor") exitWith {[east] call _updateSide};
if (CSSA3_selectedTab == "Independent") exitWith {[resistance] call _updateSide};
if (CSSA3_selectedTab == "Civilians") exitWith {[civilian] call _updateSide};
