...
zlt_fnc_partrepair = {
	...
	if (isNil {_veh} ) exitWith {}; 
	{
		...
		if (not isNil {_dmg}) then {
			...
			if (fuel _veh < 0.1) then {_veh setFuel 0.1;};
			};
		};
...
zlt_hardRepairParts = ["HitEngine", "HitLTrack","HitRTrack"] + ["HitAvionics","HitHRotor"];
	...
	if (true) then {
		{
			...
			if (not isNil {_cdmg} ) then {
				if (_cdmg > 0.7) exitWith {_flag = true};
			};
		}  forEach zlt_repair_hps;
	};
	_flag;
...
zlt_frpr_getPartsRepairTime = {
	...
	_rprTime = 0;
	{
		...
		if (not isNil {_cdmg} ) then {
			...
			if (_cdmg > 0.7) exitWith {_rprTime = _rprTime + ( if (_x in zlt_hardRepairParts) then {DEFAULT_FIELDREPAIR_EACH_HARDPART_TIME} else {DEFAULT_FIELDREPAIR_EACH_PART_TIME}); };
		};
	}  forEach zlt_repair_hps;
	_rprTime;
};
