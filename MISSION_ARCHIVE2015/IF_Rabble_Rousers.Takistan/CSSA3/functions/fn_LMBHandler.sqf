...
if (CSSA3_lmbDown) then {
	...
	if ((count crew vehicle _nearestUnit < 1) || {!(driver _nearestUnit in CSSA3_spectatableUnits)} || {!alive _nearestUnit}) exitWith {};
	...
	if !(isNull _nearestUnit) exitWith {
		...
		if (isPlayer(_nearestUnit)) exitWith {};
		
		if (side _nearestUnit == civilian) exitWith {};
		
		if (param_ai_takover == "NO") exitWith {};
		
		cameraEffectEnableHUD true;
		
		selectPlayer _nearestUnit;
		[player] join grpNull;
