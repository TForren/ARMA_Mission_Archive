...
if (isServer) then { //This scope is only for the server
	...
	[west, "BLUFOR", "player"] call FNC_AddTeam; //Adds a player team called USA on side west
	[east, "Sahrani Insurgents", "ai"] call FNC_AddTeam; //Adds a ai team called MSV on side east
...
if (!isDedicated) then { //This scope is only for the player
	...
	player setvariable ["ace_medical_medicClass", 1, true]; // Set everyone as medic
