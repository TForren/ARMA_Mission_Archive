...
else
{
	...
	private ["_transporteur", "_objets_charges", "_type_objet_a_decharger", "_objet_a_decharger"];
	...
	if !(isNull _objet_a_decharger) then
	{
		...
		if (!(_objet_a_decharger isKindOf "AllVehicles") || {_objet_a_decharger isKindOf _x} count R3F_LOG_CFG_can_be_moved_by_player > 0) then
		{
			if ({_objet_a_decharger isKindOf _x} count R3F_LOG_CFG_can_be_moved_by_player == 0) then {hintC STR_R3F_LOG_action_decharger_deplacable_exceptionnel;};
		...
		else
		{
			...
			{
				hintC STR_R3F_LOG_action_decharger_deplacable_exceptionnel;
