 ...
 * 
 * @param 0 l'usine
 * @param 1 (optionel) son crédit de création, -1 pour un crédit infinie (défaut : -1)
 * @param 2 (optionel) side pour laquelle autoriser l'accès (défaut : accès pour toutes les sides)
 * @param 3 (optionel) liste des noms de classes des catégories autorisés (white list)
...
private ["_usine", "_credits_usine", "_num_side", "_param3"];

_usine = _this select 0;

// Récupération du paramètre optionel de crédits
...
_usine addAction [("<t color=""#ff9600"">" + STR_R3F_LOG_action_ouvrir_usine + "</t>"), {_this call R3F_LOG_FNCT_usine_ouvrir_usine}, nil, 4, false, true, "", "!R3F_LOG_mutex_local_verrou && R3F_LOG_objet_addAction == _target && R3F_LOG_action_ouvrir_usine_valide"];