...
COS_distance=375;//Set spawn distance
_aerielActivation=false;// Set if flying units can activate civilian Zones

blackListTowns = ["airport_LoyManaraAirfield"];// Remove towns from COS
...
COSmotPool =["RDS_Lada_Civ_05","RDS_Lada_Civ_04","RDS_Lada_Civ_01","RDS_Lada_Civ_03","RDS_Lada_Civ_02","RDS_S1203_Civ_02","RDS_S1203_Civ_03","RDS_Gaz24_Civ_02","RDS_Golf4_Civ_01","RDS_Gaz24_Civ_03","RDS_Octavia_Civ_01","RDS_Octavia_Civ_01","RDS_Gaz24_Civ_01"];
COScivPool =["CAF_AG_ME_CIV","CAF_AG_ME_CIV_02","CAF_AG_ME_CIV_03","CAF_AG_ME_CIV_04"];

COSmaxGrps=55;//Set Maximum group limit for COS at any one time (If limit is hit then civilians will be placed into a single group for each town)
...
breakPatrol_FNC=compile preprocessFileLineNumbers "cos\patrolFnc.sqf";vehScript_FNC=compile preprocessFileLineNumbers "cos\addScript_Vehicle.sqf";
