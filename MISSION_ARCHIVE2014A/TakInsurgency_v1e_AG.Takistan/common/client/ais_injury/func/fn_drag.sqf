...
if (!isNull(_injuredperson getVariable "healer") || {!isNull(_injuredperson getVariable "dragger")} || {!isNull(_injuredperson getVariable "helper")}) exitWith {[format ["%1 is being assisted.", name _injuredperson],0, 0.035 * safezoneH + safezoneY,5,0.3] spawn BIS_fnc_dynamicText};
if (_injuredperson distance _dragger > 2.5) exitWith {[format ["%1 is too far away to be dragged.", name _injuredperson],0, 0.035 * safezoneH + safezoneY,5,0.3] spawn BIS_fnc_dynamicText};
