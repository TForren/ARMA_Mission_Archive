private ["_playerActionsCount", "_group", "_groupName", "_units", "_leader", "_maxi", "_action", "_nameArr", "_cleanNameArr", "_i", "_char", "_skipChars"];
...

{
	...
	if ((side _group == sidePlayer) && (_group != (group player))) then
	{
		...
		if ((!(isNull _leader)) && (_group getVariable ["isGroupUnlocked", true])) then
		{
			_nameArr = toArray (name _leader);
			_cleanNameArr = [];
			...
			_groupName = toString _cleanNameArr;
		...
		//test
		if ((isNull _leader) && (!(isNull _group))) then
		{
			if (((count (units _group)) > 0) && ((count ((units _group) - playableUnits)) == 0)) then
			{
				_testVarName = _group getVariable ["groupLeader", "Nil"];
				_testLeader = objNull;
				if (_testVarName != "Nil") then
				{
					_testLeader = missionNameSpace getVariable _testVarName;
				};
				player sideChat format ["DEBUG Error - Leader is %1(%2) ... varName=%3(%4)", name (leader _group), leader _group, _testVarName, if (isNull _testLeader) then {""} else {_testLeader in (units _group)}];
				player sideChat str (units _group);
			};
		};
	};
} forEach allGroups;
