...
while {true} do
{
	...
	{
		_currentUnit = _x;
		_leader = [group _currentUnit] call fnc_getLeader;
		if ((_currentUnit isKindOf "Man") && ((side _currentUnit) == sidePlayer)) then
		{
			...
			if ((alive _currentUnit) && (isPlayer _currentUnit)) then
			{
				...
				if (isACE) then
				{
					if (!([_currentUnit] call ace_sys_wounds_fnc_isUncon)) then
					{
						_currentMarker setMarkerTypeLocal "mil_triangle";
						_currentMarker setMarkerSizeLocal [0.5,1];
					}
					...
					{
						_currentMarker setMarkerTypeLocal "Dot";
						_currentMarker setMarkerSizeLocal [1,1];
					};
				};
	...
	_i = 0;
	{
		...
		) then
		{
			...
			_currentMarker setMarkerTextLocal format ["%1 (%2)", name _leader, localize format ["STR_%1s", _group getVariable "insertionType"]];
			_currentMarker setMarkerAlphaLocal 1;			
