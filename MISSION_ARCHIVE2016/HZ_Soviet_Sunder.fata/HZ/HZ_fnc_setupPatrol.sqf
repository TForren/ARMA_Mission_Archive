...
_loadouts = ["I_MG","I_AMG","I_RIF","I_AT","I_AAT","I_MARK"];
...
_patrolGroup = createGroup independent;
//_usedDuders = [];
_usedLoadouts = [];

for [{_i=0}, {_i<_teamCount}, {_i=_i+1}] do
{
	_chosenUnit = "CUP_I_TK_GUE_Soldier";
	_loadout = "";
	if !(_firstRound) then {
...
[_chosenBunkers, _patrolGroup] spawn {
	_chosenBunkers = (_this select 0) + [[7189.74,1952.83,0]]; //add tonos bay to patrol options
