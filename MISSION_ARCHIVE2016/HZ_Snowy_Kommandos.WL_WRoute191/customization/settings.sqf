...
if (isServer) then { //This scope is only for the server
	...
	setViewDistance 300; //View distance for the server (the ai's)
	
	FW_TimeLimit = 80; //Time limit in minutes, to disable the time limit set it to 0
	
	[west, "GAL", "player"] call FNC_AddTeam; //Adds a player team called USA on side west
...
if (!isDedicated) then { //This scope is only for the player
	...
	setViewDistance 400; //View distance for the player
