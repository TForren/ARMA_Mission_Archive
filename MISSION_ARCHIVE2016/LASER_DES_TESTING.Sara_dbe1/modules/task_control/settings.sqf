...
[] spawn {
	while {!FW_MissionEnded} do {
		sleep 30;
		if (!(alive mi8) && !(alive mi82)) exitwith {
			COMPLETETASK("Destroy Enemy CAS");
		};			
	};
};
[] spawn {
...
ADDTASK(west, (getmarkerpos "aamarker"), "Destroy AA", "SECONDARY OBJECTIVE: Destroy the SLA beach AA.", "AA");
