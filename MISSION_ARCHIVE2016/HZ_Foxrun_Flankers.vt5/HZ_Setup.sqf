...
if ((side player) == blufor ) then //blufor
{
	_draw3DCallsigns = addMissionEventHandler ["Draw3D",{ //staging area name tags 
			...
			if ( (player distance (getmarkerpos "HQ")) < 80) then {
				...
				drawIcon3D ["", [0.9,0.9,0.9,1], [(getpos p1) select 0,(getpos p1) select 1,((getpos p1) select 2) + 2.5], 0, 0, 0, "BANSHEE-1", 1, 0.04, "TahomaB"]; 
