...
//-------------------------------------------------------------------------------------------------------------------------
	...
	{
		//_setSkill = ["aimingAccuracy","aimingShake","aimingSpeed","Endurance","spotDistance","spotTime","courage","reloadSpeed","commanding","general"];
		_setSkill   = [[0.01,0.3],[0.4,0.6],[0.2,0.8],[0.2,0.8],[0.2,0.8],[0.2,0.8],[0.2,0.8],[0.2,0.8],[0.2,0.8],[0.2,0.8]];
		_setCombat  = ["green","white","yellow"];
...
//-------------------------------------------------------------------------------------------------------------------------
	case 2:
	{
		...
		_setForm    = ["line","column","stag column","file","diamond"];
		_setFleeing = [1,200];
		_setHeliVal = [50,100,0.7,1];
		//setPause  = time to pause at each waypoint. [min, +random] - [infantry] [wheeled] [tracked] helo[ground, air, cargo group patroling] [camps] [all units] 
		_setPause   = [[5,10],[5,10],[5,10],[20,30,5,5],[1,3],[0,0]];
		//SetBldgBeh = [units to garrison , detect radius , time to stay , time before building entered again , # of positions building must have] 
		_setBldgBeh = [4,50,200,200,4]; 
		_setPatrol  = ["45 + (20 * (skill _leader))","(60 + (random 60)) + ((skill _leader) * 50)"];
		_setSearch  = ["40 + ((skill _leader) * 150)","50 + ((skill _leader) * 50)"];
		_setSupport = [1,1];//support for other zones //opfor arti
		_setJoin    = 2;
