...
//------------------------------------Respawn Camp-----------------------------------------------------------
	case 3:
	{
		_campBasic     = ["Land_Sacks_heap_F",["Land_WoodPile_F",0,6,0],["Land_TentA_F",6,0,90],["Logic",10,15,0],0];
		_campAmmo      = [];
		_campStatic    = []; //["RDS_DSHKM_FIA", -7, 20, 0, "LOP_ChDKZ_Soldier_AR"]
		_campAddUnit   = ["LOP_AM_Soldier","LOP_AM_Soldier"];
		_campUserObj   = [["Land_TentA_F",4, 4, 45],["Land_TentA_F",-6,-2.6,-45],["Land_Sacks_heap_F",3,-6,10]];
		_campRandomObj = [[["Land_WoodenLog_F",0,50],["Land_WoodenBox_F",0,20],
							["Land_CanisterFuel_F",0,20],["Land_Canteen_F",0,10],["Land_CanisterPlastic_F",0,20]], 10, 5, 10, 2];
		_campWall      = [];
		_campObjInit   = [[],[],[],[],[],[],[]];
	};
//------------------------------------SAM Camp-----------------------------------------------------------
	case 4:
	{
		_campBasic     = ["CUP_TKLaunchers_EP1",["Land_Canteen_F",1,0,-30],["Land_WoodenLog_F",6,0,90],["Logic",10,15,0],0];
		_campAmmo      = [];
		_campStatic    = [["rhs_KORD_high_MSV", -7, 5, 0, "rhs_g_engineer_F"]]; //["RDS_DSHKM_FIA", -7, 20, 0, "LOP_ChDKZ_Soldier_AR"]  rhs_Igla_AA_pod_ins
		_campAddUnit   = ["rhs_g_Soldier_AA_F","rhs_g_Soldier_AA_F"];
		_campUserObj   = [["CUP_TKLaunchers_EP1",-2, 3, 209.528],
		["Land_TentA_F",-6,-2.6,-45],["Land_Sacks_heap_F",3,-6,10]];
		_campRandomObj = [];
		_campWall      = [];
		_campObjInit   = [[],[],[],[],[],[],[]];
	};
