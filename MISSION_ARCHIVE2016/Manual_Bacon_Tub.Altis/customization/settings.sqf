...
if (isServer) then { //This scope is only for the server
	...
	FW_TimeLimitMessage = "Time Limit Reached.<br/><br/>OPFOR VICTORY"; //The message displayed when the time runs out

	[west, "US ARMY", "player"] call FNC_AddTeam; //Adds a player team called USMC on side west
	[east, "RUS TV ", "ai"] call FNC_AddTeam; //Adds a player team called USMC on side west
