...
private _excludedUnits = _params select 9;
private _guardArray = [];
...
{
	if ((side _x == _guardSide) && !(_x in _excludedUnits)) then {
...
fnc_alertHQ = {
	...
	hint format ["HQ Alert Level %1 reported at %2", HQAlertLevel,_rtoLoc];
};//end fnc_alertHQ
...
fnc_callRTO = {
	...
	//immediately tell everyone
	{
		...
		_x setVariable ["asr_ai_exclude", false];
