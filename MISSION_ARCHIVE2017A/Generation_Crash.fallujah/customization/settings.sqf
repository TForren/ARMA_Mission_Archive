if (isServer) then { //This scope is only for the server
	...
	[independent, "Civilians", "ai"] call FNC_AddTeam; //Adds a player team called USA on side west
	[east, "Al-Qaeda", "ai"] call FNC_AddTeam; //Adds a player team called USA on side west
	[west, "US Army", "player"] call FNC_AddTeam; //Adds a ai team called MSV on side east
...
if (!isDedicated) then { //This scope is only for the player
	...
	setViewDistance 5500; //View distance for the player - set lower on urbanised map to increase player performance
