...
*/
{
	...
	if (ambCivDebug) then {
		{ //Marking roads
			if (!isNil "_x") then {
				...
				_debugMkr setMarkerType "IDM_DOT";
			};
	...
	{ //Iterate over all houses in the current town
		...
		*/
		...
		if (random 1 < civSpawnChance) then {
			...
			_wp = _fooGroup addWaypoint [[(position _newDuder select 0) + (random 20 -10),
												(position _newDuder select 1) + (random 20 -10), 0], 0];
	...
	_chosenPos = [(position _town select 0) + (random 20 -10),(position _town select 1) + (random 20 -10), 0];
		_newDuder = _townGroup createUnit ["LOP_Tak_Civ_Man_06", _chosenPos,[], 0, "FORM"];
		_newDuder disableAI "FSM";
		_newDuder call FNC_TrackUnit;
		[_newDuder] call HZ_fnc_civGear;
		if (ambCivDebug) then {
