...
[] spawn {
	sleep 1;
	_houses = nearestObjects [[7788.17,9277.62,0], ["house"], 120];
...
[] spawn {
	...
	while {!_bluNear} do {
		...
		_nearbyUnits = nearestObjects [[7813.16,9233.34,0], ["man"], 50];
		...
		if (_bluNear) then {
			{[_x, [7813.16,9233.34,0]] remoteExec ["doMove", 0, true]} foreach garrisonUnits;
		};
		sleep 10;
	};
};
