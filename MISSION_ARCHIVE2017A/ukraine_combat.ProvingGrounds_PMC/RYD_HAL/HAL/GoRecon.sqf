...
while {not _isLOS} do
	{
	_posX = ((getPosATL (leader _HQ)) select 0) + _dXb + _dXc + (random 100) - 50;
	_posY = ((getPosATL (leader _HQ)) select 1) + _dYb + _dYc + (random 100) - 50;
	...
	if (_unitG in (_HQ getVariable ["RydHQ_FOG",[]])) then 
		...
		for [{_a = 0},{_a <= 50},{_a = _a + 1}] do
			{
			_posX0 = _posX + (random 100) - 50;
			_posY0 = _posY + (random 100) - 50;
